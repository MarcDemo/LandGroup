...
@login_required
def inbox(request):
    messages = Paginator(MessageRecipient.objects.filter(recipient=request.user).select_related('message').order_by('-message__sent_at'), 20).get_page(request.GET.get('page'))
    return render(request, 'messaging/inbox.html', {'messages': messages})

@login_required
def sent_messages(request):
    messages = Paginator(Message.objects.filter(sender=request.user).order_by('-sent_at'), 20).get_page(request.GET.get('page'))
