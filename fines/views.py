...
from deposits.models import FinePaymentAllocation

# Create your views here.
@login_required
def my_fines(request):
    approved_allocations = FinePaymentAllocation.objects.filter(
        deposit__status='APPROVED',
    ).select_related('deposit__reviewed_by')
    fines = Fine.objects.active().filter(member=request.user).prefetch_related(
        Prefetch('payment_allocations', queryset=approved_allocations, to_attr='approved_allocations'),
...
def download_my_fines(request, format):
    ...
    rows = []
    for fine in fines:
        allocations = fine.approved_allocations
